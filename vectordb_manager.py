...
def parsing(path:str, crop:bool=False, lang:str="en") -> Iterator[Document]:  # 메인 Parsing 함수, text-extraction은 pypdf2 적용
    '''
        ...
        for _ in tqdm(pdf.pages):
            ...
            if len(text_result) == 0:  # 텍스트 추출 결과가 없으면, OCR 실시
                ...
                for idx in range(len(ocr_result)):
                    ...
                    except: temp_result.append("Error has been ocurred")
...
class VectordbManager:
    def parse_to_vectordb(lv1_dir:str="./DOCUMENTS"):
        ...
        for path in tqdm(total_paths):
            ...
            parsed_text = parsing(path=path, lang="en")
            ...
            print(f">>> [VectorDB End]{path}--------------------------------------------")
        


    def similarity_search(query:str, db_path:str="./chroma_langchain_db"):
...
if __name__ == "__main__":
    ...
    VectordbManager.parse_to_vectordb()

    # VectordbManager.similarity_search()
